...
const CarList = () => {
  ...
  useEffect(() => {
    const getCars = async () => {
      const carData = await fetchCar();  // Call the fetchCar function
      if (carData) {
        setCars(carData);  // Update the state with the fetched cars
      }
    };
  ...
  console.log('The car list is from CarList:', cars);
